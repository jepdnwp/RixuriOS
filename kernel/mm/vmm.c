...
static uint64_t *ensure_table(uint64_t *parent, size_t index, uint64_t flags) {
    ...
    if (e & RIXURI_PTE_PRESENT) return entry_table(e);
    uint64_t phys = pmm_alloc_page();
    if (!phys) return NULL;
    uint64_t *table = (uint64_t *)(uintptr_t)phys;
    for (size_t i = 0; i < TABLE_ENTRIES; ++i) table[i] = 0;
...
static uint64_t *split_pd_huge_page(uint64_t *pd, size_t index) {
    uint64_t e = pd[index];
    if (!(e & RIXURI_PTE_PRESENT)) return ensure_table(pd, index, 0);
    if (!(e & PTE_PS)) return entry_table(e);

    uint64_t phys = pmm_alloc_page();
    if (!phys) return NULL;
    uint64_t *pt = (uint64_t *)(uintptr_t)phys;
    uint64_t base = e & PAGE_MASK;
    uint64_t leaf_flags = e & (RIXURI_PTE_WRITE | RIXURI_PTE_USER | RIXURI_PTE_NX);
...
void vmm_early_init(void) {
    ...
    /* Keep kernel writes enabled and enable write-protect. SMEP/SMAP are
       enabled later after CPU feature probing; setting them speculatively here
       would make early user mappings unsafe. */
...
void vmm_unmap_page(uint64_t va) {
    ...
    uint64_t *pml4 = (uint64_t *)(uintptr_t)current_pml4_phys;
    uint64_t e = pml4[(va >> 39) & 0x1FFULL];
    if (!(e & RIXURI_PTE_PRESENT)) return;
    uint64_t *pdpt = entry_table(e);
    e = pdpt[(va >> 30) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return;
    uint64_t *pd = entry_table(e);
    e = pd[(va >> 21) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return;
    if (e & PTE_PS) return;
...
uint64_t vmm_translate(uint64_t va) {
    ...
    uint64_t *pml4 = (uint64_t *)(uintptr_t)current_pml4_phys;
    uint64_t e = pml4[(va >> 39) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return 0;
    uint64_t *pdpt = entry_table(e);
    e = pdpt[(va >> 30) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return 0;
    uint64_t *pd = entry_table(e);
    e = pd[(va >> 21) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return 0;
    if (e & PTE_PS) return (e & PAGE_MASK) + (va & 0x1FFFFFULL);
    uint64_t *pt = entry_table(e);
    e = pt[(va >> 12) & 0x1FFULL]; if (!(e & RIXURI_PTE_PRESENT)) return 0;
