...
typedef struct { rix_pipe_t pipe; uint8_t used; uint8_t refs; rix_waitqueue_t read_wq; } vfs_pipe_slot_t;
...
int vfs_write(uint64_t pid,int fd,const void*buffer,size_t size,size_t*out_written){VFS_GUARD();
    size_t ps;if(pid_slot(pid,&ps)||fd<0||fd>=RIX_VFS_FD_MAX||!fds[ps][fd].used||(!buffer&&size)||!out_written)return -1;if(fds[ps][fd].type==VFS_FD_PIPE_WRITE){int prc=pipe_write(fds[ps][fd].pipe,buffer,size,out_written);if(out_written&&*out_written>0){rix_pipe_t *pp=fds[ps][fd].pipe;for(size_t i=0;i<VFS_PIPE_MAX;i++)if(pipe_slots[i].used&&(&pipe_slots[i].pipe==pp)){scheduler_wake_queue(&pipe_slots[i].read_wq);break;}}return prc;}if(!fds[ps][fd].writable||fds[ps][fd].type!=RIX_VFS_FILE)return -2;rixfs_t*fs=vfs_root_fs();
if(!fs)return -3;
