...
typedef struct { uint64_t inode; uint16_t record_size; uint8_t type; uint8_t name_length; uint32_t flags; } __attribute__((packed)) rixfs_dirent_disk_t;
...
/* Moves a regular-file entry between directories; replace controls destination replacement. */
int rixfs_rename(rixfs_t *fs,uint64_t old_dir_inode,const char *old_name,uint64_t new_dir_inode,const char *new_name,int replace);
