...
typedef struct { char sig[4]; uint32_t length; uint8_t revision; uint8_t checksum; char rest[4]; } generic_sdt_t;
...
static sdt_t*find_table(const rsdp_t*r,const char*sig){
 if(r->revision>=2&&r->xsdt){sdt_t*x=(sdt_t*)(uintptr_t)r->xsdt;if(x->length<sizeof(sdt_t)||sum8(x,x->length)!=0)return NULL;size_t n=(x->length-sizeof(sdt_t))/8;uint64_t*e=(uint64_t*)((uint8_t*)x+sizeof(sdt_t));for(size_t i=0;i<n;i++){sdt_t*t=(sdt_t*)(uintptr_t)e[i];if(t&&sig4(t,sig)&&t->length>=sizeof(sdt_t)&&sum8(t,t->length)==0)return t;}}
 if(r->rsdt){sdt_t*x=(sdt_t*)(uintptr_t)(uint64_t)r->rsdt;if(x->length<sizeof(sdt_t)||sum8(x,x->length)!=0)return NULL;size_t n=(x->length-sizeof(sdt_t))/4;uint32_t*e=(uint32_t*)((uint8_t*)x+sizeof(sdt_t));for(size_t i=0;i<n;i++){sdt_t*t=(sdt_t*)(uintptr_t)(uint64_t)e[i];if(t&&sig4(t,sig)&&t->length>=sizeof(sdt_t)&&sum8(t,t->length)==0)return t;}}
 return NULL;
}
int acpi_init(uint64_t rsdp_phys){
 cpu_n=ioapic_n=mcfg_n=iso_n=0;if(!rsdp_phys)return -1;rsdp_t*r=(rsdp_t*)(uintptr_t)rsdp_phys;if(r->sig[0]!='R'||r->sig[1]!='S'||r->sig[2]!='D'||r->sig[3]!=' '||r->sig[4]!='P')return -1;size_t len=(r->revision>=2&&r->length>=sizeof(rsdp_t))?r->length:20;if(sum8(r,len)!=0)return -1;
