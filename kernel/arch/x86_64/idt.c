...
void x86_exception_dispatch(const struct interrupt_frame *frame){if(frame){serial_write("CPU exception vector=");serial_write_dec(frame->vector);serial_write(" error=");serial_write_hex(frame->error);serial_write(" rip=");serial_write_hex(frame->rip);serial_write("\r\n");}cli();for(;;)__asm__ volatile("hlt");}
void idt_init(void){
