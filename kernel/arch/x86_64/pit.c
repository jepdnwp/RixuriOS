...
void pit_irq(unsigned irq,const struct interrupt_frame *frame){(void)irq;(void)frame;ticks++;scheduler_tick();/* Phase P1-slice: 100 ms quantum, BSP-guarded inside. */if((ticks%10u)==0)scheduler_preempt_tick();}
