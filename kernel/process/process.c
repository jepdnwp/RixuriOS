...
static rix_process_t table[RIX_PROCESS_MAX];static uint32_t supplementary_groups[RIX_PROCESS_MAX][RIX_PROCESS_GROUP_MAX];static uint32_t supplementary_counts[RIX_PROCESS_MAX];static pid_t current_pid;static pid_t next_pid;static size_t live_count;
...
int process_setuid(pid_t pid,uint32_t uid){rix_process_t*p=process_lookup(pid);if(!p||p->uid!=0)return -1;p->uid=uid;return 0;}
int process_setgid(pid_t pid,uint32_t gid){rix_process_t*p=process_lookup(pid);if(!p||p->uid!=0)return -1;p->gid=gid;return 0;}
