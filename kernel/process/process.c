...
int process_init(void){
 ...
 current_pid=0;next_pid=1;live_count=1;table[0].pid=0;table[0].state=RIX_PROC_RUNNING;table[0].uid=0;table[0].gid=0;copy_name(table[0].name,"kernel");return 0;
}
...
int process_create_user(const char *name,pid_t parent,const void *image,uint64_t image_size,uint64_t *out_entry,uint64_t *out_user_stack){
 if(!name||!name[0]||!image||!image_size||!out_entry||!out_user_stack)return -1;if(parent!=0&&!process_lookup(parent))return -1;
 pid_t pid=0;if(process_create(name,parent,&pid)!=0)return -1;rix_process_t *p=process_lookup(pid);if(!p)return -1;
 if(address_space_create(&p->address_space)!=0)goto fail;
 for(uint64_t i=0;i<USER_STACK_PAGES;i++){
  uint64_t pa=pmm_alloc_page();if(!pa)goto fail;zero_page(pa);
...
fail:
 ...
 if(p->kernel_stack){pmm_free_page(p->kernel_stack);p->kernel_stack=0;p->kernel_stack_size=0;}
