...
int process_create_session(pid_t pid,pid_t*session){rix_process_t*p=process_lookup(pid);if(!p||p->state==RIX_PROC_UNUSED||p->state==RIX_PROC_ZOMBIE||p->process_group==pid)return -1;p->session=pid;p->process_group=pid;if(session)*session=pid;return 0;}
...
int process_leave_session(pid_t pid){rix_process_t*p=process_lookup(pid);if(!p||p->state==RIX_PROC_UNUSED||p->state==RIX_PROC_ZOMBIE)return -1;p->session=0;p->process_group=pid;return 0;}
