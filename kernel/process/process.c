...
/* Parallel credential arrays (real/saved IDs, supplementary groups, audit
 * UIDs) are keyed by TABLE SLOT, never by PID: allocate_pid() hands out
 * PIDs round-robin, so after any reap/reuse cycle pid != slot. Indexing
 * these arrays by pid reads a live-or-dead stranger's credentials (observed:
 * audit_uid inheritance failing once smoke-test forks advance next_pid past
 * the reused slots). All accessors below resolve the slot first. */
static long slot_of(pid_t pid){for(size_t i=0;i<RIX_PROCESS_MAX;i++)if(table[i].state!=RIX_PROC_UNUSED&&table[i].pid==pid)return (long)i;return -1;}
...
int process_setuid(pid_t pid,uint32_t uid){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||slot<0)return -1;if(p->uid==0&&((p->capabilities&RIX_CAP_SETUID)==0))return -1;if(p->uid==0){real_uids[slot]=uid;saved_uids[slot]=uid;p->uid=uid;if(uid!=0)p->capabilities=0;return 0;}
if(uid==real_uids[slot]||uid==saved_uids[slot]){p->uid=uid;return 0;}return -1;}
int process_setgid(pid_t pid,uint32_t gid){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||slot<0)return -1;if(p->uid==0&&((p->capabilities&RIX_CAP_SETGID)==0))return -1;if(p->uid==0){real_gids[slot]=gid;saved_gids[slot]=gid;p->gid=gid;return 0;}
if(gid==real_gids[slot]||gid==saved_gids[slot]){p->gid=gid;return 0;}return -1;}
int process_in_group(pid_t pid,uint32_t gid){rix_process_t*p=process_lookup(pid);if(!p)return 0;if(p->gid==gid)return 1;long slot=slot_of(pid);if(slot>=0)for(size_t i=0;i<supplementary_counts[slot];i++)if(supplementary_groups[slot][i]==gid)return 1;return 0;}
int process_getgroups(pid_t pid,uint32_t*groups,size_t capacity,size_t*count){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||!count||slot<0||(capacity&&capacity<supplementary_counts[slot])||(!groups&&capacity))return -1;if(groups&&capacity)for(size_t i=0;i<supplementary_counts[slot];i++)groups[i]=supplementary_groups[slot][i];*count=supplementary_counts[slot];return 0;}
int process_setgroups(pid_t pid,const uint32_t*groups,size_t count){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||p->uid!=0||(p->capabilities&RIX_CAP_SETGID)==0||slot<0||count>RIX_PROCESS_GROUP_MAX||(count&&!groups))return -1;
for(size_t i=0;i<count;i++){
        supplementary_groups[slot][i]=groups[i];
    }
    supplementary_counts[slot]=(uint32_t)count;
    return 0;
}
int process_apply_exec_credentials(pid_t pid,uint32_t uid,uint32_t gid,int setuid_bit,int setgid_bit){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||slot<0)return -1;if(setuid_bit){p->uid=uid;saved_uids[slot]=uid;}if(setgid_bit){p->gid=gid;saved_gids[slot]=gid;}if(setuid_bit||setgid_bit)p->capabilities=0;return 0;}
static int process_create_locked(const char*name,pid_t parent,pid_t*out_pid){if(!name||!name[0]||(parent&& !process_lookup(parent)))return -1;for(size_t i=1;i<RIX_PROCESS_MAX;i++)if(table[i].state==RIX_PROC_UNUSED){pid_t pid=allocate_pid();if(!pid)return -1;clear_process(&table[i]);supplementary_counts[i]=0;audit_uids[i]=0;real_uids[i]=saved_uids[i]=real_gids[i]=saved_gids[i]=0;table[i].pid=pid;table[i].parent=parent;rix_process_t*pp=parent?process_lookup(parent):NULL;table[i].process_group=pp?pp->process_group:pid;table[i].session=pp?pp->session:pid;if(pp){copy_cwd(table[i].cwd,pp->cwd);table[i].uid=pp->uid;table[i].gid=pp->gid;long parent_slot=slot_of(parent);if(parent_slot>=0){real_uids[i]=real_uids[parent_slot];saved_uids[i]=saved_uids[parent_slot];real_gids[i]=real_gids[parent_slot];saved_gids[i]=saved_gids[parent_slot];audit_uids[i]=audit_uids[parent_slot];supplementary_counts[i]=supplementary_counts[parent_slot];for(size_t g=0;g<supplementary_counts[i];g++)supplementary_groups[i][g]=supplementary_groups[parent_slot][g];}}else audit_uids[i]=table[i].uid;table[i].state=RIX_PROC_SLEEPING;table[i].capabilities=pp?pp->capabilities:(parent==0?RIX_CAP_ALL:0);if(parent&&vfs_clone_fds(parent,pid)!=0){(void)vfs_close_all(pid);clear_process(&table[i]);return -1;}copy_name(table[i].name,name);live_count++;if(out_pid)*out_pid=pid;return 0;}return -1;}
...
int process_exec_user_with_args(pid_t pid, const void *image, uint64_t image_size,
    ...
    /* Phase R1: exec replaces the address space, so a second live thread
     * would dangle. No clone syscall exists yet (Phase 25), hence at most
     * the execing thread is ACTIVE — but the rule is enforced, not
     * assumed. Takes sched_lock under PROCESS_GUARD (PROC4->sched edge,
     * same as the exit path). */
    if (scheduler_thread_count_for_pid((uint64_t)pid) > 1) return -1;
...
int process_signal_group(pid_t process_group,unsigned signal){PROCESS_GUARD();if(!process_group||signal<1u||signal>64u)return -1;uint64_t bit=1ULL<<(signal-1u);int sent=0;for(size_t i=0;i<RIX_PROCESS_MAX;i++){rix_process_t*p=&table[i];if(p->state!=RIX_PROC_UNUSED&&p->state!=RIX_PROC_ZOMBIE&&p->process_group==process_group){pid_t target=p->pid;p->signal_pending|=bit;if(p->state==RIX_PROC_SLEEPING&&(p->signal_mask&bit)==0)p->state=RIX_PROC_RUNNING;scheduler_wake_pid((uint64_t)target);sent++;}}return sent?0:-1;}
...
static int wait_reap_locked(rix_process_t *match,uint64_t *status,pid_t *child_pid){/* Phase R1: refcounted ownership — the address space is destroyed only when no ACTIVE thread of the child remains. Returns 1 (same as "no zombie yet": blocking callers re-block, polling callers retry) while a racing thread still lives; that thread's bootstrap refuses the ZOMBIE root and frees itself, so the gate is self-clearing. DETACHED threads keep pre-R1 semantics (they die at the next activation, which refuses ZOMBIE roots) and never gate the reap. */if(scheduler_thread_count_for_pid((uint64_t)match->pid)!=0)return 1;long dead_slot=match-table;pid_t session=match->session;*status=match->exit_status;*child_pid=match->pid;(void)vfs_close_all(match->pid);address_space_destroy(&match->address_space);if(match->kernel_stack)pmm_free_page_range(match->kernel_stack,KERNEL_STACK_PAGES);clear_process(match);if(dead_slot>=0&&(size_t)dead_slot<RIX_PROCESS_MAX){audit_uids[dead_slot]=0;real_uids[dead_slot]=saved_uids[dead_slot]=real_gids[dead_slot]=saved_gids[dead_slot]=0;supplementary_counts[dead_slot]=0;}if(live_count)live_count--;session_drop_if_empty(session);return 0;}
...
int process_get_audit_uid(pid_t pid,uint32_t*out){rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||!out||slot<0)return -1;*out=audit_uids[slot];return 0;}
int process_set_audit_uid(pid_t pid,uint32_t uid){PROCESS_GUARD();rix_process_t*p=process_lookup(pid);long slot=slot_of(pid);if(!p||slot<0||!process_has_capability(pid,RIX_CAP_AUDIT_ADMIN))return -1;audit_uids[slot]=uid;return 0;}
