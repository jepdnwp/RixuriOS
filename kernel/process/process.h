...
/* Prepare a fresh user bootstrap without switching CR3 on the old kernel
 * stack; the user-entry trampoline performs that first load immediately
 * before IRETQ. Subsequent scheduler resumes use process_activate(). */
int process_activate_user_entry(pid_t pid);
...
int process_list_sessions(rix_session_info_t *out,size_t capacity,size_t *count);
int process_list_processes(rix_process_info_t *out,size_t capacity,size_t *count);
