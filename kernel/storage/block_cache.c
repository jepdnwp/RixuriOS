...
int block_cache_read(rix_block_device_t*d,uint64_t s,void*b){
 if(!d||!b||d->sector_size==0||d->sector_size>CACHE_SECTOR_BYTES)return -1;uint64_t f=0;rix_spin_lock_irqsave(&cache_lock,&f);cache_entry_t*e=find(d,s);if(e){e->age=++clock_tick;for(uint32_t i=0;i<d->sector_size;i++)((uint8_t*)b)[i]=e->data[i];rix_spin_unlock_irqrestore(&cache_lock,f);return 0;}e=victim();int dirty=e->valid&&e->dirty;rix_block_device_t*old_d=e->device;uint64_t old_s=e->sector;uint8_t old_data[CACHE_SECTOR_BYTES];if(dirty)for(uint32_t i=0;i<d->sector_size;i++)old_data[i]=e->data[i];invalidate(e);rix_spin_unlock_irqrestore(&cache_lock,f);
...
int block_cache_write(rix_block_device_t*d,uint64_t s,const void*b){
 if(!d||!b||d->sector_size==0||d->sector_size>CACHE_SECTOR_BYTES)return -1;uint8_t input[CACHE_SECTOR_BYTES];for(uint32_t i=0;i<d->sector_size;i++)input[i]=((const uint8_t*)b)[i];uint64_t f=0;rix_spin_lock_irqsave(&cache_lock,&f);cache_entry_t*e=find(d,s);if(e){for(uint32_t i=0;i<d->sector_size;i++)e->data[i]=input[i];e->dirty=1;e->age=++clock_tick;rix_spin_unlock_irqrestore(&cache_lock,f);return 0;}e=victim();int dirty=e->valid&&e->dirty;rix_block_device_t*old_d=e->device;uint64_t old_s=e->sector;uint8_t old_data[CACHE_SECTOR_BYTES];if(dirty)for(uint32_t i=0;i<d->sector_size;i++)old_data[i]=e->data[i];invalidate(e);rix_spin_unlock_irqrestore(&cache_lock,f);if(dirty&&io(old_d,RIX_BIO_WRITE,old_s,old_data)!=0)return -1;
