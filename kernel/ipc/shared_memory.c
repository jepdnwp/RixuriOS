...
int shm_map(rix_shm_id_t id,pid_t pid,uint64_t va,uint64_t flags){
 shm_object_t*o=lookup(id);rix_process_t*p=process_lookup(pid);if(!o||!p||!p->address_space.pml4_phys||!va||(va&4095ULL)||va<0x0000008000000000ULL||va>=0x0000800000000000ULL||o->size>0x0000800000000000ULL-va)return -1;
