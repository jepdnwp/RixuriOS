...
void syscall_dispatch(rix_syscall_frame_t*frame){
 ...
 case RIX_SYS_POLL:{typedef struct {int fd;short events;short revents;} rix_pollfd_t;rix_process_t*p=process_lookup(self);size_t count=(size_t)frame->rsi,ready=0;if(!p||count>64u){result=-RIX_EINVAL;break;}for(size_t i=0;i<count;++i){rix_pollfd_t item;if(copy_from_user(&item,frame->rdi+i*sizeof(item),sizeof(item))!=0){result=-RIX_EFAULT;break;}item.revents=0;if(rix_net_socket_poll_ready(&p->sockets,item.fd,item.events,&item.revents)!=0){result=-RIX_EINVAL;break;}if(item.revents)++ready;if(copy_to_user(frame->rdi+i*sizeof(item),&item,sizeof(item))!=0){result=-RIX_EFAULT;break;}result=(int64_t)ready;}break;}
