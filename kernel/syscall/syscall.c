...
void syscall_dispatch(rix_syscall_frame_t*frame){
 ...
 case RIX_SYS_EXECVE:{char path[RIX_VFS_PATH_MAX];if(user_string(frame->rdi,path,sizeof(path))!=0){result=-RIX_EFAULT;break;}int fd;if(vfs_open(self,path,0,0,&fd)!=0){result=-RIX_EINVAL;break;}void *image=kmalloc(RIX_MAX_EXEC_IMAGE,16u);size_t image_size=0;if(!image||vfs_read(self,fd,image,RIX_MAX_EXEC_IMAGE,&image_size)!=0){(void)vfs_close(self,fd);result=-RIX_EFAULT;break;}(void)vfs_close(self,fd);uint64_t entry,stack;if(process_exec_user(self,image,image_size,&entry,&stack)!=0){result=-RIX_EINVAL;break;}if(process_activate(self)!=0){result=-RIX_EINVAL;break;}frame->rip=entry;frame->rsp=stack;result=0;break;}
