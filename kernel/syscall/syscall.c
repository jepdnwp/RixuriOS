...
void syscall_dispatch(rix_syscall_frame_t*frame){
 ...
 case RIX_SYS_MKDIR:{char path[RIX_VFS_PATH_MAX];if(user_string(frame->rdi,path,sizeof(path))!=0){result=-RIX_EFAULT;break;}result=vfs_mkdir(path,(uint32_t)frame->rsi,process_uid(self),process_gid(self))==0?0:-RIX_EINVAL;break;}
 ...
 case RIX_SYS_LINK:{char old_path[RIX_VFS_PATH_MAX],new_path[RIX_VFS_PATH_MAX];if(user_string(frame->rdi,old_path,sizeof(old_path))!=0||user_string(frame->rsi,new_path,sizeof(new_path))!=0){result=-RIX_EFAULT;break;}result=vfs_link(old_path,new_path)==0?0:-RIX_EINVAL;break;}
 ...
 case RIX_SYS_CHMOD:{char path[RIX_VFS_PATH_MAX];if(user_string(frame->rdi,path,sizeof(path))!=0){result=-RIX_EFAULT;break;}result=vfs_chmod(path,(uint32_t)frame->rsi)==0?0:-RIX_EINVAL;break;}
 ...
 case RIX_SYS_CHDIR:{char input[RIX_VFS_PATH_MAX],resolved[RIX_VFS_PATH_MAX];rix_vfs_path_t path;if(user_string(frame->rdi,input,sizeof(input))!=0||vfs_normalize_path(input,resolved,sizeof(resolved))!=0||vfs_lookup(resolved,&path)!=0||!path.node||path.node->type!=RIX_VFS_DIR||process_setcwd(self,resolved)!=0)result=-RIX_EINVAL;else result=0;break;}
