...
void syscall_dispatch(rix_syscall_frame_t*frame){
 ...
 case RIX_SYS_CONNECT:{rix_process_t*p=process_lookup(self);rix_net_endpoint_t endpoint;if(!p||copy_from_user(&endpoint,frame->rsi,sizeof(endpoint))!=0){result=-RIX_EFAULT;break;}result=rix_net_socket_connect(&p->sockets,(int)frame->rdi,endpoint);break;}
 ...
 case RIX_SYS_RECV:{rix_process_t*p=process_lookup(self);rix_net_endpoint_t endpoint;uint8_t buffer[RIX_NET_MTU];size_t capacity=(size_t)frame->rdx;if(!p||capacity>sizeof(buffer)){result=-RIX_EINVAL;break;}result=rix_net_socket_receive(&p->sockets,(int)frame->rdi,buffer,capacity,frame->r10?&endpoint:0);if(result>=0&&(copy_to_user(frame->rsi,buffer,(size_t)result)!=0||(frame->r10&&copy_to_user(frame->r10,&endpoint,sizeof(endpoint))!=0)))result=-RIX_EFAULT;break;}
