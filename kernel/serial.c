...
void serial_write(const char *s){if(!s||!serial_has_com1)return;while(*s)serial_putc(*s++);}
void serial_write_n(const char *s,size_t length){if(!s||!serial_has_com1)return;for(size_t i=0;i<length;i++)serial_putc(s[i]);}
