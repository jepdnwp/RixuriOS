...
int elf_load_image(const void*file,uint64_t file_size,rix_address_space_t*as,rix_elf_image_t*out){
 ...
 const uint8_t*b=(const uint8_t*)file;const Elf64_Ehdr*h=(const Elf64_Ehdr*)b;const Elf64_Phdr*ph=(const Elf64_Phdr*)(b+h->e_phoff);uint64_t lo=UINT64_MAX,hi=0;
 for(uint16_t i=0;i<h->e_phnum;i++){
  if(ph[i].p_type!=1)continue;
  if(ph[i].p_memsz==0||ph[i].p_vaddr<0x1000||!range(ph[i].p_vaddr,ph[i].p_memsz,1ULL<<47)||ph[i].p_offset>file_size||ph[i].p_filesz>file_size-ph[i].p_offset||ph[i].p_filesz>ph[i].p_memsz)return -1;
  uint64_t end=ph[i].p_vaddr+ph[i].p_memsz;if(end<ph[i].p_vaddr)return -1;
  if(ph[i].p_align>1&&((ph[i].p_align&(ph[i].p_align-1))!=0||((ph[i].p_vaddr-ph[i].p_offset)&(ph[i].p_align-1))!=0))return -1;
  uint64_t first=ph[i].p_vaddr&~0xfffULL,last=(end+0xfffULL)&~0xfffULL;if(last<end)return -1;
  uint64_t wanted=RIXURI_PTE_PRESENT|RIXURI_PTE_USER|RIXURI_PTE_NX;if(ph[i].p_flags&2)wanted|=RIXURI_PTE_WRITE;if(ph[i].p_flags&1)wanted&=~RIXURI_PTE_NX;
  for(uint64_t va=first;va<last;va+=4096){
   uint64_t old=address_space_query_flags(as,va);uint64_t pa=address_space_translate(as,va)&~0xfffULL;int owned=(old&RIXURI_PTE_OWNED)!=0;
