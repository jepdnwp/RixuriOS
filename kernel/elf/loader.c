...
int elf_load_image(const void*file,uint64_t file_size,rix_address_space_t*as,rix_elf_image_t*out){
 ...
 for(uint16_t i=0;i<h->e_phnum;i++){
  ...
  for(uint64_t va=first;va<last;va+=4096){
   uint64_t pa=address_space_translate(as,va)&~0xfffULL;uint64_t old=address_space_query_flags(as,va);int existing=pa!=0;
   uint64_t flags=wanted;
   if(existing){
    flags|=old&RIXURI_PTE_WRITE;
    if(!(wanted&RIXURI_PTE_NX)||!(old&RIXURI_PTE_NX))flags&=~RIXURI_PTE_NX;
   }else{pa=pmm_alloc_page();if(!pa)return -1;zero_page((uint8_t*)(uintptr_t)pa);}
   if(address_space_map(as,va,pa,flags)!=0){if(!existing)pmm_free_page(pa);return -1;}
