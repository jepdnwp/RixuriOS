...
int symlink(const char *target,const char *path){return(int)rix_int_result(rix_sys(85,(long)target,(long)path,0));}
rix_ssize_t readlink(const char *path,char *buffer,size_t capacity){return(rix_ssize_t)rix_int_result(rix_sys(88,(long)path,(long)buffer,(long)capacity));}
...
int listen(int fd,int backlog){if(backlog<0){errno=RIX_EINVAL;return -1;}return(int)rix_int_result(rix_sys(154,fd,backlog,0));}
int accept(int fd,struct sockaddr *address,socklen_t *length){rix_net_endpoint_t peer={0,0};long rc=rix_sys(155,fd,(long)&peer,0);if(rc<0)return(int)rix_int_result(rc);if(address&&endpoint_to_sockaddr(peer,address,length)!=0)return -1;return(int)rc;}
