...
int program_main(int argc,char **argv,char **envp){
    ...
    {int listener=socket(AF_INET,SOCK_STREAM,0),client=-1,accepted=-1;struct sockaddr_in listen_addr={0};listen_addr.sin_family=AF_INET;listen_addr.sin_port=htons(42001);listen_addr.sin_addr.s_addr=htonl(INADDR_LOOPBACK);char payload[]="accept-data",received[32]={0};int ok=listener>=0&&bind(listener,(struct sockaddr*)&listen_addr,sizeof(listen_addr))==0&&listen(listener,2)==0&&accept(listener,0,0)<0&&errno==RIX_EAGAIN;if(ok){client=socket(AF_INET,SOCK_STREAM,0);ok=client>=0&&connect(client,(struct sockaddr*)&listen_addr,sizeof(listen_addr))==0;accepted=ok?accept(listener,0,0):-1;ok=ok&&accepted>=0&&send(client,payload,sizeof(payload),0)==(ssize_t)sizeof(payload)&&recv(accepted,received,sizeof(received),0)==(ssize_t)sizeof(payload)&&memcmp(payload,received,sizeof(payload))==0&&listen(listener,99)<0&&errno==RIX_EINVAL;}if(accepted>=0)close(accepted);if(client>=0)close(client);if(listener>=0)close(listener);check("listen",ok);}
