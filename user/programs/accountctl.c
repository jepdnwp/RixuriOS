...
static int build_passwd(const char *input,const char *name,uint32_t uid,uint32_t gid,int mode,char *output,size_t cap) {
    ...
    while(cursor&&*cursor){const char *next=next_line(cursor,line,sizeof(line));if(!next)return-1;if(!line_name(line,name)){if(append_text(output,cap,&used,line)||append_text(output,cap,&used,"\n"))return-1;}else{found=1;if(mode==1){if(append_text(output,cap,&used,name)||append_text(output,cap,&used,":" )||append_uint(output,cap,&used,uid)||append_text(output,cap,&used,":" )||append_uint(output,cap,&used,gid)||append_text(output,cap,&used,":/home/")||append_text(output,cap,&used,name)||append_text(output,cap,&used,":/bin/sh\n"))return-1;}else if(mode!=2){if(append_text(output,cap,&used,line)||append_text(output,cap,&used,"\n"))return-1;}}cursor=next;}
    if(mode==0&&!found){if(append_text(output,cap,&used,name)||append_text(output,cap,&used,":" )||append_uint(output,cap,&used,uid)||append_text(output,cap,&used,":" )||append_uint(output,cap,&used,gid)||append_text(output,cap,&used,":/home/")||append_text(output,cap,&used,name)||append_text(output,cap,&used,":/bin/sh\n"))return-1;}
