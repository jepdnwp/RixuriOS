...
build/rixfs.img: programs scripts/build-rixfs-image.py etc/hosts etc/hostname etc/resolv.conf | build
	python3 scripts/build-rixfs-image.py -o $@ \
		...
		--file /usr/bin/pipetest=build/programs/pipetest.elf \
		--file /usr/bin/schedtest=build/programs/schedtest.elf \
		--file /usr/bin/threads=build/programs/threads.elf \
		--file /usr/bin/crashtest=build/programs/crashtest.elf \
		--file /usr/bin/fuzztest=build/programs/fuzztest.elf \
		--file /usr/bin/cloexec-test=build/programs/cloexec-test.elf \
		--file /usr/bin/epipe-test=build/programs/epipe-test.elf \
		--file /bin/readlink=build/programs/readlink.elf \
		--file /bin/chmod=build/programs/chmod.elf \
		--file /bin/chown=build/programs/chown.elf \
		--file /sbin/reboot=build/programs/reboot.elf \
		--file /sbin/poweroff=build/programs/poweroff.elf \
...
hub-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/usb_hub_test.c kernel/usb/hub.c -o build/usb_hub_test
	build/usb_hub_test
storage-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/usb_storage_test.c kernel/usb/storage.c -o build/usb_storage_test
	build/usb_storage_test
storage-retry-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/usb_storage_retry_test.c kernel/usb/storage.c -o build/usb_storage_retry_test
	build/usb_storage_retry_test
storage-submit-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/usb_storage_submit_test.c kernel/usb/storage.c kernel/storage/block.c -o build/usb_storage_submit_test
	build/usb_storage_submit_test
tty-test: | build
...
xhci-caps-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. tests/xhci_caps_test.c kernel/usb/xhci/caps.c -o build/xhci_caps_test
		build/xhci_caps_test

xhci-profile-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. tests/xhci_profile_test.c kernel/usb/xhci/profile.c -o build/xhci_profile_test
	build/xhci_profile_test

xhci-portsc-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. tests/xhci_portsc_test.c -o build/xhci_portsc_test
	build/xhci_portsc_test

xhci-linux-derived-test: | build
...
xhci-cc4-snapshot-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/xhci_cc4_snapshot_test.c kernel/usb/xhci/debug.c -o build/xhci_cc4_snapshot_test
	build/xhci_cc4_snapshot_test

xhci-dma-boundary-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. -Iinclude tests/xhci_dma_boundary_test.c -o build/xhci_dma_boundary_test
	build/xhci_dma_boundary_test
...
thread-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -DRIX_HOST_TEST -I. tests/thread_test.c kernel/sched/thread.c -o build/thread_test
		build/thread_test
...
dma-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -DRIX_HOST_TEST -I. tests/dma_test.c kernel/pci/dma.c kernel/mm/pmm.c kernel/sync/lock.c kernel/sync/lockdep.c -o build/dma_test
		build/dma_test

nvme-prp-test: | build
	$(HOST_CC) -std=c17 -Wall -Wextra -Werror -I. tests/nvme_prp_test.c kernel/storage/nvme_prp.c -o build/nvme_prp_test
		build/nvme_prp_test
...
test: check usb-test hid-test hub-test storage-test storage-retry-test storage-submit-test tty-test shell-test pipe-test net-test libc-test hosts-test rtl-test e1000-test acpi-test smp-test gdt-test pmm-test heap-test xhci-caps-test xhci-profile-test xhci-portsc-test xhci-linux-derived-test xhci-ep-interval-test xhci-cc4-snapshot-test xhci-dma-boundary-test rixfs-mount-test symlink-test sync-test thread-test runqueue-test block-cache-test elf-test dma-test nvme-prp-test statfs-test klog-test abi-check
...
clean:
	rm -rf build kernel/*.o kernel/mm/*.o kernel/arch/x86_64/*.o kernel/pci/*.o kernel/sched/*.o kernel/process/*.o kernel/syscall/*.o kernel/vfs/*.o kernel/fs/*.o kernel/elf/*.o kernel/sync/*.o kernel/storage/*.o kernel/log/*.o kernel/usb/*.o kernel/usb/xhci/*.o kernel/ipc/*.o kernel/tty/*.o kernel/time/*.o kernel/power/*.o
