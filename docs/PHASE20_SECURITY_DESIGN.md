...
The continuation evidence gate requires `cap=PASS`, `acl=PASS`, `matrix=PASS`, `setid=PASS`, `kill=PASS` and `session=PASS`, with no page fault, CPU exception or kernel panic markers. The shell prompt remains intentionally deferred until the broader Phase 20 security boundary is complete.
