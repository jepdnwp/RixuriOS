...
| Block/NVMe recovery and persistence ordering are incomplete | 11–13, 19, 20 | block cache, NVMe, RixFS source | NVMe now has one bounded reset/admin-I/O queue reinitialization path after repeated timeout/stuck completion; physical fault injection, interrupt-driven recovery, and filesystem durability remain UNVERIFIED. |
