...
| Block/NVMe recovery and persistence ordering are incomplete | 11–13, 19, 20 | block cache, NVMe, RixFS source | NVMe now has one bounded reset/admin-I/O queue reinitialization path after repeated timeout/stuck completion. RixFS fsck repair now reconstructs only bitmap mismatches after full structural validation; physical fault injection, interrupt-driven recovery, transaction repair, and filesystem durability remain UNVERIFIED. |
