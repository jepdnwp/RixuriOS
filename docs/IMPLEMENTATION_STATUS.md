...
- TTY: canonical/raw input, line readiness, echo/output queues, foreground process-group state, PTY master/slave byte/line flow, terminal dimensions, bounded ANSI/VT screen state and control-signal delivery are implemented and host-tested; full session ownership remains open.
