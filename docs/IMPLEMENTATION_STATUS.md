...
- Added bounded `RIX_SYS_RENAME`/`rename()` support for regular files within one directory. The journaled dirent update preserves inode identity and metadata, rejects an existing destination with `-EEXIST`, and makes `/bin/mv` use rename first before its metadata-preserving copy fallback.
- Added `/usr/bin/renametest`, which verifies inode-preserving rename, destination collision failure and a rename round trip in QEMU.
- Added persistent `/etc/passwd` and root-readable-only `/etc/shadow` fixtures, with a bounded `rixsha256` salted/iterated password format. `/usr/bin/authcheck` validates account records, correct and incorrect password outcomes, and non-root denial of shadow reads.
- Added the reproducible `auth-test` target and included account/password evidence in `make phase20-test`.
- Added a bounded in-memory persistent session registry with `RIX_SYS_LIST_SESSIONS`, returning session ID, leader, UID and controlling-TTY state through a capacity-checked snapshot. Session records are created for user-init and `setsid`/login sessions, and are removed only after the last live member leaves.
- Added `/usr/bin/sessionlisttest` and extended the session QEMU harness to prove two concurrent sessions are observable and that logout/exit cleanup removes the child session without disturbing the parent session.
- Clean strict build, host tests, Phase 20 credential/CAP_KILL QEMU, session lifecycle/registry QEMU, atomic rename, metadata cp/mv and account/authentication QEMU suites all passed in this environment.

## Phase 20 audit identity continuation — 2026-09-07

- Added an out-of-line PID-indexed audit UID initialized from the creator identity and inherited across fork/spawn without changing the stable `rix_process_t` layout.
- Added version-1 `get_audit_uid` and `set_audit_uid` syscalls plus freestanding libc wrappers. Assignment requires `CAP_AUDIT_ADMIN`; dropping that capability fails closed with `-EACCES`.
- Ordinary `execve` preserves the audit UID, including through the existing set-ID credential path. Set-ID execution clears effective capabilities but does not rewrite the recorded audit identity.
...
## Power-loss, ISO and unified test runner — 2026-09-07

A reproducible QEMU crash-injection harness now copies a disposable RixFS image, kills the guest at four write timings across account add, password rotation and account removal, reboots the same image, replays the journal and verifies credential recovery. The 12-case matrix passed. A bootable UEFI El Torito ISO is generated by `make iso`, and `make iso-test` boots that ISO with the RixFS NVMe image and verifies kernel readiness and shell startup.

The new `make test-all` runner produces a timestamped full log under `build/test-logs/`, runs the strict host suite, ISO build/boot, Phase 20 suite, power-loss matrix and each QEMU regression. Its first complete run recorded 20 PASS and 1 FAIL. The failing `qemu_pipe_stress_test.py` exposed an existing kernel page fault in the pipe/fork stress path (`PAGE FAULT`, child pid 3); the runner intentionally reports this instead of masking it. This remains an open regression to fix before claiming an all-green suite.
...
## Phase 21 network stack continuation — 2026-09-08

The first protocol-composition slice of Phase 21 is implemented and validated within the deterministic loopback boundary. Added bounded UDP datagram construction/parsing with IPv4 pseudo-header checksum validation, including length, port and payload bounds. Extended the TCP module with wire-segment construction/parsing, sequence/acknowledgment fields, flag validation, minimum data-offset checks and IPv4 pseudo-header checksum validation while retaining the explicit TCP state machine.

The socket loopback path no longer fabricates an HTTP response directly from a string match. TCP connect now runs a real SYN → SYN/ACK → ACK exchange through the segment serializer/parser and reaches `ESTABLISHED` only after validating the returned sequence and acknowledgment values. A loopback HTTP service accepts only a validated ACK/PSH `GET` segment and queues a validated ACK/PSH response segment; malformed or non-GET traffic does not produce a success response. UDP socket send/receive similarly serializes and parses a datagram before delivery. The raw ICMP loopback path validates the request checksum and constructs an echo reply through the ICMP packet helpers.

Host `net-test` coverage now includes UDP checksum and payload round trips, checksum corruption rejection, TCP SYN and ACK/PSH segment round trips, TCP stateful loopback HTTP exchange, ICMP behavior, ARP expiry, Ethernet framing and existing socket error cases. The strict kernel/userspace build, host USB/HID/TTY/shell/pipe/network/NIC tests, image generation and QEMU loopback harnesses passed. Real QEMU evidence includes `E1000: probe`, `RIXURI:KERNEL_READY`, `/usr/bin/ping` reporting `ping: 127.0.0.1: PASS`, and `/usr/bin/curl` reporting `curl: HTTP 200 loopback PASS`.

The evidence boundary remains explicit. QEMU E1000 currently proves PCI discovery, BAR/MMIO mapping and DMA-ring configuration only; the driver is not yet connected to the protocol/socket path with validated TX/RX completion. No DNS, DHCP, routing-table, ARP-on-wire, IPv4-on-NIC, UDP-on-NIC, TCP-on-NIC, interrupt/recovery or physical RTL8125 result is claimed. The external-network QEMU harness now reaches the shell and verifies fail-closed `ping: DNS/network path unavailable` and `curl: DNS/network path unavailable` results; it does not count those errors as network success. HTTPS/TLS is unsupported by design at this phase.
...
## Physical AMD bring-up slice — 2026-09-09

Target machine Ryzen 7 7700 + ASUS PRIME B650M-R hangs silently after `RIXURI:KERNEL_READY` (no `USER_ENTER`), with `ACPI: unavailable`, `xHCI: controllers=0`, `NET: no usable network device` and `VFS: mount nvme0n1 rc=5` on the serial log. Code analysis (no hardware access in sandbox) found and fixed the following bring-up hazards without changing any QEMU success path:

- ACPI: `find_table` scanned at most 16 XSDT/RSDT entries, which rejects firmware tables larger than that (typical on this board). The scan now covers the full validated table. `acpi_init` returns staged codes (`ACPI_ERR_*` in `acpi.h`) and the boot line names the failing stage (`ACPI: unavailable (<reason>)`); new `tests/acpi_test.c` covers a 24-entry XSDT with the MADT last, plus bad signature/checksum/missing/truncated cases.
- PCI: new `pci_print_devices()` prints one compact line per device (`bus:dev.fn vendor:device class/sub/prog bar0 cmd`); called once from `kernel_main`.
- xHCI: probe now sizes BAR0 via `pci_bar_size` with decode quiesced, validates base/size (16 MB sanity cap), maps the real size, performs the USBLEGSUP BIOS→OS handoff with bounded walks/polls (previously absent — a hang risk on firmware-owned controllers), reports every skip reason, and prints `xHCI: controller=N PCI=... BAR=... operational=... ports=...` on success. AMD fallback list gains `15B8` (Windows-visible xHCI ID on this board); class-code matching stays primary.
- Boot markers: `BOOT: ACPI begin/done`, `BOOT: PCI done`, `BOOT: NVMe done`, `BOOT: VFS mount done`, `BOOT: xHCI begin/done`, `BOOT: scheduler begin`, and `BOOT: user entry pid=... entry=... stack=... pml4=...` in `task_bootstrap` (distinguishes "scheduler never switched" from "iretq faulted", since faults already print diagnostics).
- NVMe/VFS: `rc=-5` is decoded as "not a RixFS superblock" (expected on the Windows NTFS disk; mount failure does not block the embedded init). `try_mount_root` now also logs a `BLOCK:` inventory and per-namespace `NVMe: ctrl/ns/sectors/sector_size` lines (read-only). No partition support exists (`nvme0n1p1` names never resolve) — out of scope and untouched.
- RTL8125: fixed four bring-up blockers — TX descriptors programmed NIC-owned at init (first transmit always returned busy; now driver-owned with EOR preserved), `program_rings` read never-initialized `tx_dma`/`rx_dma` instead of `tx_ring_phys`/`rx_ring_phys`, missing TX doorbell (`TPPOLL/NPQ` write added to transmit), and missing RX accept filter (`RCR` programmed to unicast+multicast+broadcast). `link_up` field widened to `int` (also fixes the GCC 14 `-Werror=incompatible-pointer-types` build break). `tests/rtl8125_test.c` extended (OWN states, doorbell write, RCR value, ring programming via `ring_phys`, TX/RX round-trip on heap pages) plus missing host stubs; the target previously neither compiled nor linked.
- `kernel/net/device.c` now selects the E1000 backend first and falls back to RTL8125, logging `NET: backend=<name>`; the E1000 transmit/receive path is byte-identical. `kernel_main` probes both NICs.
