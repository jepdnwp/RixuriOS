...
IMAGE = ROOT / "build" / "rixfs-cloexec.img"
ESP = ROOT / "build" / "uefi" / "esp-cloexec"
LOG = ROOT / "build" / "qemu-cloexec.log"
...
try:
    ...
    time.sleep(1.0)
    send(b"/usr/bin/cloexec-test")
    if not read_until(b"cloexec:PASS", 40.0):
        raise RuntimeError("cloexec PASS marker not observed")
    if not read_until(b"\x1b[1;37m:\x1b[0m ", 10.0):
        raise RuntimeError("shell prompt did not return after cloexec test")
finally:
    proc.terminate()
    try:
        proc.wait(timeout=3)
