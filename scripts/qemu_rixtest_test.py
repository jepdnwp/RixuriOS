...
IMAGE = ROOT / "build" / "rixfs-rixtest.img"
ESP = ROOT / "build" / "uefi" / "esp-rixtest"
LOG = ROOT / "build" / "qemu-rixtest.log"
...
try:
    ...
    time.sleep(1.0)
    cmd = b"/usr/bin/rixtest" + (b" " + mode.encode() if mode else b"")
    send(cmd)
    if mode in ("--all", "--fuzz"):
        timeout = 600.0
    else:
        timeout = 300.0
    if not read_until(b"summary: ", timeout):
        raise RuntimeError("rixtest summary not observed")
    if not read_until(b"\x1b[1;37m:\x1b[0m ", 15.0):
        raise RuntimeError("shell prompt did not return after rixtest")
finally:
    proc.terminate()
    try:
        proc.wait(timeout=3)
