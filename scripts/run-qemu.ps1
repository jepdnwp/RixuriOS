...
# Firmware lives outside build\uefi on purpose: build-uefi.sh wipes that
# tree on every image build, which used to delete these files silently.
$OvmfDir = Join-Path $Root 'build\firmware\ovmf'
...
if (-not (Test-Path $OvmfCode) -or -not (Test-Path $OvmfVars)) {
    ...
    wsl -u root -- bash -c "cp /usr/share/OVMF/OVMF_CODE_4M.fd /usr/share/OVMF/OVMF_VARS_4M.fd '$wslDir/build/firmware/ovmf/'"
}
