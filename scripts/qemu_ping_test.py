...
try:
    ...
    proc.stdin.write(b"/usr/bin/ping\n")
    proc.stdin.flush()
    if not until(b"ping: 127.0.0.1: PASS", 20):
        raise RuntimeError("ping PASS marker missing")
    if not until(b"\x1b[1;37m:\x1b[0m ", 10):
        raise RuntimeError("prompt missing after ping")
finally:
    proc.terminate()
    try:
        proc.wait(timeout=3)
